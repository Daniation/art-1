...
public class SellerOrdersView {
    ...
    public static BorderPane getView() {
        ...
        Text pageTitle = new Text("Orders for My Artworks");
        ...
        if (orders.isEmpty()) {
            Text emptyText = new Text("No orders for your artworks yet.");
            ...
            for (Order order : orders) {
                ...
                if ("Pending".equals(currentStatus)) {
                    ...
                    shipBtn.setOnAction(e -> updateStatus(orderService, order.getId(), "Shipped", ordersList));
                    actions.getChildren().add(shipBtn);

                    Button cancelBtn = new Button("Cancel Order");
                    cancelBtn.getStyleClass().add("remove-btn");
                    cancelBtn.setOnAction(e -> updateStatus(orderService, order.getId(), "Cancelled", ordersList));
                ...
                if ("Shipped".equals(currentStatus)) {
                    ...
                    deliverBtn.setOnAction(e -> updateStatus(orderService, order.getId(), "Delivered", ordersList));
    ...
    private static void updateStatus(OrderService service, int orderId, String newStatus, VBox ordersList) {
        if (AlertUtil.showConfirm("Update Status", "Change order status to " + newStatus + "?")) {
            if (service.updateOrderStatus(orderId, newStatus)) {
                AlertUtil.showInfo("Updated", "Order #" + orderId + " status: " + newStatus);
